...
df = pd.DataFrame({'Age Group': compas_scores_grouped_age['age_cat'], 'Proportion': compas_scores_grouped_age['proportion']})
show(df)
...
# Display table 
df = pd.DataFrame({'Race': compas_scores_grouped_race['race'], 'Proportion': compas_scores_grouped_race['proportion']})
show(df)
...
model = sm.Logit(y, X.astype(float)).fit() # uses statsmodels.api
...
iterations = 10000 # Number of jury panels to simulate
...
plt.title("Simulation: Number of Black Citizens in 100-Person Jury Panel", fontsize=16)
...
<iframe width="320" height="100" src="https://vclock.com/embed/timer/#countdown=00:04:00&amp;enabled=0&amp;seconds=300&amp;theme=0&amp;ampm=1&amp;sound=xylophone" frameborder="0" allowfullscreen>
...
- Ethical practices that data scientists can engage while working with data from observational studies include: 
...
plt.title("Dashed lines indicate total admission rate by gender", fontsize=13)
plt.xlabel("Department", fontsize=15)
plt.ylabel("Admission Rate", fontsize=15)
